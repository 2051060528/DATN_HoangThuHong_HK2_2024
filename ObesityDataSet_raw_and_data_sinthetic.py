...
with st.form(key='my_form'):
    ...
    with col1:
        age = st.slider('**Tuổi**', 14, 61)
        height = st.slider('**Chiều cao (m)**', 1.45, 1.98)
        weight = st.slider('**Cân nặng (kg)**', 39.0, 173.0)
...
if submit_button:
    ...
    predicted_obesity_level = model.predict(user_input_data)
        ...
        # Map the predicted label to its corresponding category and message
